...
# START_DTP = RUNWAY_END_DTP_VALUE * 0.55
CTE_LIMIT = 15
HE_LIMIT = 90
NUM_SECTIONS = 4
    ...
    with XPlaneConnect() as client:
        ...
        with tqdm(total=job_params['end_DTP']) as pbar:
            while dtp < job_params['end_DTP']:
                ...
                # FIXME: Add an exception handler for when the getHomeState function fails
                # # If the getHomeState function fails, try again for timeout seconds
                # # If it fails again, then break out of the loop
                # timeout = 5
                # while timeout > 0:
                #     try:
                #         _, dtp, _ = getHomeState(client)
...
def find_duplicates(jobs, existing_jobs):
    duplicate_jobs = []
    for job in jobs:
        for existing_job in existing_jobs:
            if job['start_CTE'] == existing_job['start_CTE'] and \
                job['start_HE'] == existing_job['start_HE'] and \
                job['time_of_day'] == existing_job['time_of_day'] and \
                job['cloud_cover'] == existing_job['cloud_cover'] and \
                job['end_DTP'] == existing_job['end_DTP']:
                duplicate_jobs.append(job)
                break
    
    for duplicate_job in duplicate_jobs:
        jobs.remove(duplicate_job)
...
def main():
    ...
    #     # time_of_day_range=(8.0, 12.0),
    #     # time_of_day_range=(12.0, 16.0),
    #     # time_of_day_range=(16.0, 24.0),
    #     cloud_cover_range=(0, 4),
    #     # cloud_cover_range=(0, 1),
    #     # cloud_cover_range=(2, 4),
    #     end_DTP_perc=0.45,
    #     # end_DTP_perc=0.95,
    #     max_jobs=1000
    # )
    ...
    # # Optional: compare with existing job list
    # with open(OUT_DIR.parent.joinpath('20230609-120514_max_jobs_1000_todr_all_ccr_all_erdp_45_p1').joinpath('jobs.pkl'), 'rb') as f:
    #     existing_jobs = pickle.load(f)
    
    # duplicate_jobs = find_duplicates(jobs, existing_jobs)

    # print(f'existing jobs: {len(existing_jobs)}')
    # print(f'duplicate jobs: {len(duplicate_jobs)}')
    # print(f'generated jobs (no duplicates): {len(jobs)}')

    # # print(f'generated jobs: {jobs}')


    # ====================Load jobs==========================
    # jobs_path = OUT_DIR.parent.joinpath('20230609-120514_max_jobs_1000_todr_all_ccr_all_erdp_45').joinpath('jobs.pkl')
    jobs_path = OUT_DIR.parent.joinpath('20230913-124805_max_jobs_1000_todr_all_ccr_all_erdp_45_p2').joinpath('jobs.pkl')
    with open(jobs_path, 'rb') as f:
        jobs = pickle.load(f)
    
    jobs = jobs[900:1000]
    # edtp = 0.45
    # jobs = jobs[500:509] # TODO: pytorchx-pl
    # jobs = jobs[600:900]
    # jobs = jobs[958:999]
    # jobs = [jobs[999]]
...
if __name__ == "__main__":
    ...
    # argparser.add_argument('--jobs_path', type=Path, help='Path to the jobs file (default: None).')
    # argparser.add_argument('--num_jobs', type=int, help='Number of jobs to generate (default: ).')
